...
$parcelCount = (array_values(array_filter(array_column($parcelDetails,'orderprocess')))); 
		...
		<button class="btn btn-block btn-default" onclick="runningOrder('1')" >Dine In <span class="ml-1 badge badge-success"><?= count($dineIn) ?? 0; ?></span></button>
		</div>
	<!--	<div class="col-md-4 text-center">
		<button class="btn orderactive btn-block" onclick="runningOrder('2')">New Order <span class="ml-1 badge badge-success"><?= count($newOrders) ?? 0; ?></span></button>
		</div>-->
		<div class="col-md-6 text-center">
		<button class="btn orderactive btn-block" onclick="runningOrder('2')">Takeaway <span class="ml-1 badge badge-success"><?= count($parcelCount) ?? 0; ?></span></button>
		</div>
		</div>
				</div>
				</div>
          ...
          <div class="col-lg-12">
            <div class="card">
              ...
              <div class="card-header d-flex align-items-center">
                <h3 class="h4 float-left col-md-8 pl-0">Takeaway</h3>
              ...
              <div class="card-body p-3">
				...
				<?php for($i=0;$i<count($parcelDetails);$i++){

					  $table_status = ($parcelDetails[$i]['table_status'] ?? 5);
					?>
					<div class="col-md-4 filtersearch food-order">
                <div class="tile active">
				<div class="row">
				<div class="col-md-8">
				<?= $parcelDetails[$i]['tablename'];  ?>
				</div>
				<div class="col-md-4">
				<?= date('h:i A',strtotime($parcelDetails[$i]['reg_date']));  ?>
				</div>
				</div>
				<div class="row">
				<div class="col-md-8">
				<?= Utility::orderstatus_details($parcelDetails[$i]['orderprocess']) ; ?> 
				</div>
				<div class="col-md-4">
				<?= '₹ '.round($parcelDetails[$i]['totalamount'],2);  ?>
				</div>
				</div>
				<div class="row">
				<div class="col-md-8">
				...
				<div class="col-md-4">
				<button type="button" class="badge badge-success mb-0" onclick="billview('<?= $parcelDetails[$i]['ID'] ;?>');"><i class="fa fa-eye" title="BILL"></i></button>
				</div>
				</div>
					<div class="row">
				<div class="col-md-8">
				...
				<div class="col-md-4">
							<?php if($parcelDetails[$i]['ordertype'] == 1){ echo 'Online'; }else{  echo 'Offline'; }  ?> 
				...
				<div class="col-md-4">
					 <?php $pilotName = \app\helpers\Utility::serviceboy_details($parcelDetails[$i]['serviceboy_id'],'name');  
					 if(!empty($pilotName)){
					     echo $pilotName;
					 }else{
					     echo 'UnAssigned';
				...
				<?php if($parcelDetails[$i]['orderprocess'] != '4' && $parcelDetails[$i]['orderprocess'] != '3'){ ?>
				<div class="text-center mt-1">
	<!--			<button class="placeorder btn" onclick="tableorderstatuschange('<?= $parcelDetails[$i]['ID'] ;?>','PARCEL','<?= $parcelDetails[$i]['orderprocess'] ;?>')">Status</button>  -->
					<button class="placeorder btn" onclick="changepilotstatus('PARCEL','<?= $parcelDetails[$i]['ID'] ;?>','4','')">Completed</button>
				<button class="btn btn-danger btn-xs placeordercnl" onclick="changepilotstatus('PARCEL','<?= $parcelDetails[$i]['ID'] ;?>','3','')">Cancelled</button>
