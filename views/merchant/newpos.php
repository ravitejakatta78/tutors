                ...
                <div class="row">
                  ...
                  <div class="buts mt-2 text-center col-md-12">
                    ...
                    <?php } else if (!empty($prevFullSingleOrderDet) && @$prevFullSingleOrderDet['orderprocess'] == 1 &&  @$prevFullSingleOrderDet['preparetime'] == 0) { ?>
                      <button class="btn btn-Info mb-2 fnt" data-toggle="modal" onclick="openprepmodal('<?= !empty($prevOrderDetails) ? $prevOrderDetails[0]['order_id'] : ""; ?>')">Start Preparation</button>
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8">
                <select id="discounttype" name="discounttype">
                  ...
                  <option value="1" <?php if (@$prevFullSingleOrderDet['discount_type'] == '1') {
                                      echo 'selected';
                                    } ?>>Overall</option>
                  <option value="2" <?php if (@$prevFullSingleOrderDet['discount_type'] == '2') {
                                      echo 'selected';
                                    } ?>>Percentage</option>
                </select>
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8">
                <input type="text" id="merchant_coupon" placeholder="Apply Coupon" class="form-control" value="<?= $prevFullSingleOrderDet['coupon'] ?? ''; ?>" <?php if (!empty($prevFullSingleOrderDet['coupon'])) {
                                                                                                                                                                  echo 'readonly';
                                                                                                                                                                } ?>>
              </div>
            </div>
...
<div id="myModalLess" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="form-group row">
              <label class="control-label cr-label col-md-4">OTP</label>
              <div class="col-md-8">
                  <input type="text" value="" name="lessotp" id="lessotp" class="form-control">
                  <input type="hidden" value="" name="hiddenotp" id="hiddenotp" class="form-control">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" id="savelessotp">Save</button>
      </div>
    </div>
  </div>
</div>
...
<script>
    ...
    return function(total_modified_name) {
      ...
      if (quantity_arr.length >= 2) {
        ...
        if ($("#quant_id_" + quantity_arr[0]['ID']).val() == 0) {
          ...
          checktoadd(quantity_arr[0]['ID'], quantity_arr[0]['price'], quantity_arr[0]['food_type_name'] || '', 3);
        } else {
  ...
  function saveorder() {
    ...
    if (isNaN(ttl_sub_amt)) {
      ...
      $("#hiddenotp").val('');
      $("#hiddenotp").val(sent_otp);
    });
      $("#myModalLess").modal('show');
      return false;
    }
  ...
  function checkCouponCode() {
    var request = $.ajax({
      ...
      type: "POST",
      data: {
        'id': $('#merchant_coupon').val(),
        'discount_amount': $(".ttl-cpn-amt").html(),
        'sub_total_amount': $(".ttl-sub-amt").html(),
        'mobile_number': $("#usermobile").val(),
  ...
  $("#savelessotp").click(function(){
      var lessotp = $("#lessotp").val();
      var hiddenotp = $("#hiddenotp").val();
      if(lessotp != hiddenotp){
        swal(
              'Warning!',
              'Please Provide A Valid OTP',
              'warning'
