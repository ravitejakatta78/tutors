		...
		if(!empty($val['merchantid'])){
					...
					->where(['status' => '1', 'ID' => $merchantid])->asArray()->One();
					 if(!empty($merchantsdata)){
                            ...
                            $feedbackrating = Yii::$app->db->createCommand($sqlfeedbackrating)->queryOne();
                             $merchants['rating'] = !empty($feedbackrating) ? number_format($feedbackrating['rating'],2) : 0;
                            ...
                            foreach($allAmenites as $amenityId => $amenity){
                                ...
                                if(!empty($amenityVisibilityArray[$amenityId]) && @$amenityVisibilityArray[$amenityId] == 1){
                                    $amenitySingleArray['isShow'] = true;
                                }
                                else{
                                    $amenitySingleArray['isShow'] = false;
                                }
    ...
    try {
		...
		if(!empty($tabel_Det)){
						...
						$valtax = !empty($val['tax']) ? number_format(trim($val['tax']), 2, '.', ',') : 0;
							...
							$userarray['serviceboy_id'] = isset($val['serviceboy_id']) ? $val['serviceboy_id'] : null;
	...
	public function reordercash($val){
		...
		if(!empty($val['merchantid'])&&!empty($val['table'])&&!empty($val['productid'])&&!empty($val['count'])&&!empty($val['price'])){ 
						...
						$orderamount = trim($val['totalamount']);
							...
							$userarray['amount'] =  $orderdetails['amount']+$amountant;
